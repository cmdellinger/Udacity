...
# 58 to 62
# Note: use the graph that you created from the previous exercise to see the answer. You do not need to run summaries.
```
...
ggplot(aes(x = volume, y = price),
       data = subset(diamonds, volume < quantile(volume, 0.9))) +
...
```{r Greatest prop_initiated Group}
# On average, which group initiated the greatest proportion of its Facebook friendships? The plot with teh smoother that you created in the last exercise can help you answer this question.

# The people that joined later seem to initiated more of their friendships. Which means, (2012,2014] initiated the most.
```
...
pf %>%
  ...
  summarize(proportion = mean(prop_initiated)) %>%
  arrange(year_joined.bucket)
# Why do you think this group's proportion of friendships initiated is higher than the others?

#The groups joining later already had friends on the service to add, whereas the people to add as friends were limited when there was a smaller user population. In other words, the new users added the people they already knew on the service, so as more users joined, there were more users to add at the time of account creation, making the amount of friends initiated larger.
```
