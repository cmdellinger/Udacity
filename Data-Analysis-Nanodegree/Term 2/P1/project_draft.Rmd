...
is assumed to be borrower's who wanted to obstain from reporting or having it
...
```{r echo=FALSE,  message=FALSE, warning=FALSE, StatedMonthlyIncome_Histogram}
ggplot(aes(x = StatedMonthlyIncome), data = loanData) +
  geom_histogram(binwidth = 12500) +
  scale_x_continuous(limits = c(0,175000)) +
  scale_y_log10() +
  labs(x = 'Logarithm of Stated Monthly Income', y = 'N')
```
...
the catch alls of "Other" and "Professional" are ambiguous and seem to 
...
The loans seem to be grouped in three descrete lengths.
...
The table comfirms that the data set contains only three loan lengths, which
...
maximum is 10. Interestingly, the cleaner plot hides the discreteness of the 
variables.
...
```{r echo=FALSE,  message=FALSE, warning=FALSE, primary_variables_grid}
...
```{r echo=FALSE,  message=FALSE, warning=FALSE, ggpairs_auxiliary}
ggpairs(data = loanData,
...
# create a discrete variable to make the plot look nicer
#
...
# leaving Prosper score as gradient because discrete shader runs out of colors
ggplot(data = loanData,
       aes(x = EstimatedReturn,
           y = BorrowerAPR,
           color = ProsperScore)) +
...
correlation with the Prosper score, which is the company's own credit score.
Although there
